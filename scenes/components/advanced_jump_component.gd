...
@export var jump_mass_exponent: float = 0.6
...
func _get_scaled_jump_velocity(magnet: MagnetComponent) -> float:
	var multiplier = pow(2.0 / magnet.getMass(), jump_mass_exponent)
	return max(jump_velocity * multiplier, min_jump_velocity)
...
func handle_jump(body: CharacterBody2D, want_to_jump: bool, jump_released: bool, magnet: MagnetComponent) -> void:
	...
	handle_jump_buffer(body, want_to_jump, magnet)
...
func jump(body: CharacterBody2D, magnet: MagnetComponent) -> void:
	body.velocity.y = _get_scaled_jump_velocity(magnet)
...
func handle_jump_buffer(body: CharacterBody2D, want_to_jump: bool, magnet: MagnetComponent) -> void:
